    ...
    public partial class Form1
    {
        ...
        private void LoadMembershipTypes()
        {
            try
            {
                ...
                var rows = _allMembershipTypesGrid.Select(m => new MembershipGridRow
                {
                    MembershipTypeID = m.MembershipTypeID,
                    Membership = m.PackageName + " (" + m.MembershipTypeID + ")",
                    Price = m.Price,
                    DurationDays = m.DurationDays,
                    MaxReservationHoursPerMonth = m.MaxReservationHoursPerMonth,
                    MeetingRoomAccess = m.MeetingRoomAccess,
                    MeetingRoomHours = m.MeetingRoomHoursPerMonth?.ToString() ?? "-"
                }).ToList();

                dgvMembershipTypes.DataSource = null;
                dgvMembershipTypes.DataSource = rows;
        ...
        private void FormatMembershipsGrid()
        {
            ...
            if (dgvMembershipTypes.Columns.Contains("MembershipTypeID"))
                dgvMembershipTypes.Columns["MembershipTypeID"].Visible = false;

            if (dgvMembershipTypes.Columns.Contains("Membership"))
                dgvMembershipTypes.Columns["Membership"].HeaderText = "Membership";
            ...
            if (dgvMembershipTypes.Columns.Contains("MeetingRoomHours"))
                dgvMembershipTypes.Columns["MeetingRoomHours"].HeaderText = "Meeting Room Hours";
        }

        private void dgvMembershipTypes_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvMembershipTypes.CurrentRow == null) return;
            if (dgvMembershipTypes.CurrentRow.DataBoundItem is not MembershipGridRow row) return;

            MembershipType membership = _allMembershipTypesGrid.FirstOrDefault(x => x.MembershipTypeID == row.MembershipTypeID);
        ...
        private void btnMembershipAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                LoadMembershipTypes();
                ClearMembershipForm();
                NotifyObservers("Memberships", "Add", "Dodat je novi tip članstva: " + membership.PackageName + ".");
            }
            catch (Exception ex)
            {
                MessageBox.Show(
        ...
        private void btnMembershipUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                LoadMembershipTypes();
                ClearMembershipForm();
                NotifyObservers("Memberships", "Update", "Izmenjen je tip članstva ID=" + membership.MembershipTypeID + ".");
            }
            catch (Exception ex)
            {
                MessageBox.Show(
        ...
        private void btnMembershipDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                LoadMembershipTypes();
                ClearMembershipForm();
                NotifyObservers("Memberships", "Delete", "Obrisan je tip članstva ID=" + membershipId + ".");
            }
            catch (Exception ex)
            {
                MessageBox.Show(
