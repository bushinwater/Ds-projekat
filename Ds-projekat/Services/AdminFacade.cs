...
namespace Ds_projekat.Services
{
    internal class AdminFacade
    {
        ...
        public Admin Login(string username, string password)
        {
            ...
            if (string.IsNullOrWhiteSpace(admin.HashedPass))
                throw new Exception("Admin postoji, ali lozinka u bazi nije upisana.");

            string[] parts = admin.HashedPass.Split('.');
            if (parts.Length != 3)
                throw new Exception("Admin postoji, ali lozinka u bazi nije hashovana odgovarajućim formatom.");

            if (!PasswordHelper.VerifyPassword(password.Trim(), admin.HashedPass))
        ...
        public List<Admin> GetAll()
        {
            ...
            {
                throw new Exception("Greška pri dohvatanju admina: " + ex.Message);
            }
        }
    }
}